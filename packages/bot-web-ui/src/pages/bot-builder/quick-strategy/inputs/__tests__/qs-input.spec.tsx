...
describe('<QSInput />', () => {
    ...
    it('should increase the value on click of "+" button', async () => {
    ...
    it('should decrease the value on click of "-" button', async () => {
        mock_store.ui.is_mobile = false;
    ...
    it('the onMouseLeave handler sets the focus to false', async () => {
        render(<QSInput name='duration' type='number' onChange={onChange} />, {
            wrapper,
        });

        const container_element = screen.getByTestId('dt_qs_input_container');
        await userEvent.unhover(container_element);

        expect(container_element).not.toHaveClass('no-focus');
    });

    it('the onMouseEnter handler sets the focus to false if the error or focus on element is absent ', async () => {
        mock_store.ui.is_mobile = false;
        ...
        const container_element = screen.getByTestId('dt_qs_input_container');
        await userEvent.type(container_element, '{enter}');

        expect(container_element).not.toHaveClass('no-focus');
    });

    it('the decrease should round a decimal number to two decimal places', async () => {
        render(<QSInput name='size' type='number' onChange={onChange} />, {
            wrapper,
        });

        const btn_element = screen.getByTestId('dt_qs_input_decrease');
        await userEvent.click(btn_element);
    ...
    it('the increase icon should round a decimal number to two decimal places', async () => {
    ...
    it('should be able to call handleOnChange() when type equals number and display the same number', async () => {
        render(<QSInput name='duration' type='number' onChange={onChange} />, {
            wrapper,
        });
        const input_element = screen.getByTestId('dt_qs_duration');
        await userEvent.type(input_element, '1');
        expect(input_element).toHaveDisplayValue(['1']);
    });

    it('should be able to call handleOnChange() when type equals text by default and display default number', async () => {
        render(<QSInput name='duration' onChange={onChange} />, {
            wrapper,
        });
        const input_element = screen.getByTestId('dt_qs_duration');
        await userEvent.type(input_element, 'a');
        expect(input_element).toHaveDisplayValue(['1']);
    });

    it('should display a number of not more than 12 digits and be able to call handleOnChange() when type equals number, max length equals or more than 12', async () => {
        render(<QSInput name='loss' type='number' onChange={onChange} />, {
            wrapper,
        });
        const input_element = screen.getByTestId('dt_qs_loss');
        await userEvent.type(input_element, '9');
