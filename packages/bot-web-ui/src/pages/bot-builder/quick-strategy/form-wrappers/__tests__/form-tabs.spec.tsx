...
describe('<FormTabs />', () => {
    ...
    it('should not call onChange on click if the tab is disabled', async () => {
    ...
    it('should not call onChange on Enter keydown if the tab is disabled', async () => {
        const { container } = render(<FormTabs {...mock_props} onChange={() => jest.fn()} />, { wrapper });
        const disabledTab = screen.getByText(FORM_TABS[1].label);
        await userEvent.type(disabledTab, '{enter}');

        expect(container).not.toBeEmptyDOMElement();
        expect(onChangeMock).not.toHaveBeenCalled();
    });

    it('should call onChange on click if the tab is not disabled', async () => {
        const { container } = render(<FormTabs {...mock_props} />, { wrapper });
    ...
    it('should call onChange on Enter keydown if the tab is not disabled', async () => {
        const { container } = render(<FormTabs {...mock_props} />, { wrapper });

        const enabledTab = screen.getByText(FORM_TABS[0].label);

        await userEvent.type(enabledTab, '{enter}');
