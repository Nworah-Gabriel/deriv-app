...
describe('WorkspaceGroup', () => {
    ...
    it('should call toggleLoadModal function on clicking import icon', async () => {
        ...
        await userEvent.click(import_button);
    ...
    it('should call onSortClick function on clicking sort icon', async () => {
        ...
        await userEvent.click(sort_button);
        expect(mockOnSortClick).toBeCalledTimes(1);
    });

    it('should call setChartModalVisibility function on clicking charts icon', async () => {
        mock_store.ui.is_desktop = true;
        render(<WorkspaceGroup />, {
            wrapper,
        });
        const chart_button = screen.getByTestId('dt_toolbar_chart_button');
        await userEvent.click(chart_button);
        expect(mockChartModalVisibility).toBeCalledTimes(1);
    });

    it('should call setTradingViewModalVisibility function on clicking tradingview chart icon', async () => {
        mock_store.ui.is_desktop = true;
        render(<WorkspaceGroup />, {
            wrapper,
        });
        const tradingview_chart_button = screen.getByTestId('dt_toolbar_tradingview_chart_button');
        await userEvent.click(tradingview_chart_button);
    ...
    it('should call onUndoClick function with false on clicking undo icon', async () => {
        ...
        await userEvent.click(undo_button);
    ...
    it('should call onZoomInOutClick function with true on clicking zoomIn icon', async () => {
        ...
        await userEvent.click(zoom_button);
