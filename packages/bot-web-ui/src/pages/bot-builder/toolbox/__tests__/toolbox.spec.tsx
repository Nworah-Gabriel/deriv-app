...
describe('Toolbox', () => {
    ...
    it('should render setVisibility to be false on blocks menu click', async () => {
        ...
        await userEvent.click(blocks_menu);
        expect(screen.getByTestId('db-toolbox__content-wrapper')).not.toHaveClass('active');
    });

    it('should render setFormVisibility to be true on quick strategy button is clicked', async () => {
        render(<Toolbox />, { wrapper });

        const quick_strategy_button = screen.getByText('Quick strategy');
        await userEvent.click(quick_strategy_button);
        expect(mock_DBot_store?.quick_strategy.is_open).toBeTruthy();
    });

    it('should render flyout when block menu category is clicked', async () => {
        render(<Toolbox />, { wrapper });
        const block_menu_category = screen.getByText('Trade parameters');

        await userEvent.click(block_menu_category);
        expect(mock_DBot_store?.flyout.is_visible).toBeTruthy();
    });

    it('should not render flyout when block menu subcategory is clicked', async () => {
        render(<Toolbox />, { wrapper });
        const block_menu_subcategory = screen.getByText('Utility');

        await userEvent.click(block_menu_subcategory);
        expect(mock_DBot_store?.flyout.is_visible).toBeFalsy();
    });

    it('should render flyout when block menu subcategory item is clicked', async () => {
        render(<Toolbox />, { wrapper });
        const block_menu_subcategory = screen.getByText('Utility');
