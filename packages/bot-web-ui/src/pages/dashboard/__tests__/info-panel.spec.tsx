...
describe('InfoPanel', () => {
    ...
    it('should render the closing of the info panel on close icon click', async () => {
    ...
    it('should render the switching of tabs upon clicking the text link', async () => {
        render(<InfoPanel />, { wrapper });

        const guide_tab = screen.getByText('Deriv Bot - your automated trading partner');
        await userEvent.click(guide_tab);
        expect(mock_DBot_store?.dashboard.setActiveTab(DBOT_TABS.TUTORIAL));
    });

    it('should render the tutorial tab and FAQ tab', async () => {
        render(<InfoPanel />, { wrapper });

        const faq_tab = screen.getByText('What is Deriv Bot?');
        await userEvent.click(faq_tab);
        expect(mock_DBot_store?.dashboard.setActiveTabTutorial(1));
    });

    it('should not render to tutorial tab and FAQ tab if no link is present', async () => {
        render(<InfoPanel />, { wrapper });
