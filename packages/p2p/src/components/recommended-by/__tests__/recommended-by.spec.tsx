...
describe('<RecommendedBy />', () => {
    it('it should show `No one has recommended this trader yet` and `recommended_average` equals to 0% if there is no props passed', async () => {
        render(<RecommendedBy />);
        await userEvent.click(screen.getByTestId('dt_popover_wrapper'));
        expect(screen.getByText('No one has recommended this trader yet')).toBeInTheDocument();
        expect(screen.getByText('0%')).toBeInTheDocument();
    });

    it('it should show `Recommended by 1 trader` and `recommended_average` equals to 100%', async () => {
    ...
    it('it should show `Recommended by 2 traders` and `recommended_average` equals to 50%', async () => {
    ...
    it('should call showModal function in mobile view', async () => {
