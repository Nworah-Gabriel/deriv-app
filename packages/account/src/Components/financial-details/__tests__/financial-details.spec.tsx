...
describe('<FinancialDetails />', () => {
    ...
    it('should trigger "Previous" or "Submit" button', async () => {
        ...
        await userEvent.type(account_turnover_select as HTMLElement, 'account turnover 1');

        await userEvent.type(education_level_select as HTMLElement, 'education level 2');
        await userEvent.type(employment_indystry_select as HTMLElement, 'employment industry 1');
        await userEvent.type(estimated_worth_select as HTMLElement, 'estimated worth 2');
        await userEvent.type(income_source_select as HTMLElement, 'income source 1');
        await userEvent.type(net_income_select as HTMLElement, 'net income 1');
        await userEvent.type(occuppation_select as HTMLElement, 'Government Officers');

        await userEvent.type(source_of_wealth_select as HTMLElement, 'source of wealth 1');
    ...
    it('should change the selected value when user changes the value in the dropdown', async () => {
        (useDevice as jest.Mock).mockReturnValue({ isDesktop: false });
        ...
        await userEvent.selectOptions(income_source_select as HTMLElement, 'Salaried Employee');
    ...
    it('should show "Unemployed" in occupation list if employment status is not "Employed"', async () => {
        ...
        await userEvent.type(occuppation_select as HTMLElement, 'Unemployed');
