...
describe('<TermsOfUse/>', () => {
    ...
    it('should allow users to accept or reject FATCA declaration ', async () => {
        render(<TermsOfUse {...mock_props} />);
        const el_fatca_form = screen.getByText('Please select*');

        await userEvent.click(el_fatca_form);
    ...
    it('should enable add account button only if spain residence confirmation checkbox is checked for spain clients', async () => {
        mock_props.residence = 'es';
        ...
        await userEvent.click(pep_checkbox);
        await userEvent.click(terms_and_condition_checkbox);
        ...
        await userEvent.click(spain_checkbox);
