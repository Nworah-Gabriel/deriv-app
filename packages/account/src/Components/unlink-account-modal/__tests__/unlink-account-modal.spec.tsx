...
describe('UnlinkAccountModal', () => {
    ...
    it('should invoke onClickSendEmail when clicking on Unlink button', async () => {
        render(<UnlinkAccountModal {...mock_props} />);

        await userEvent.click(screen.getByText('Unlink from Google'));
        expect(mock_props.onClickSendEmail).toHaveBeenCalledTimes(1);
        expect(mock_props.onClose).toHaveBeenCalledTimes(1);
    });

    it('should invoke onClose when clicking on Cancel button', async () => {
        render(<UnlinkAccountModal {...mock_props} />);
