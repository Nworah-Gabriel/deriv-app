...
describe('ApiTokenDeleteButton', () => {
    ...
    it('should display Delete this token when mouse enter', async () => {
        renderAPIDeleteButton();
        const delete_icon = screen.getByTestId('dt_token_delete_icon');
        await userEvent.hover(delete_icon);
        expect(screen.getByText(delete_this_token_text)).toBeInTheDocument();
    });

    it('should not display Delete this token when mouse leave', async () => {
        renderAPIDeleteButton();
        const delete_icon = screen.getByTestId('dt_token_delete_icon');
        await userEvent.hover(delete_icon);
        expect(screen.getByText(delete_this_token_text)).toBeInTheDocument();
        await userEvent.unhover(delete_icon);
        expect(screen.queryByText(delete_this_token_text)).not.toBeInTheDocument();
    });

    it('should display Popup when delete icon is clicked', async () => {
        renderAPIDeleteButton();
        const delete_icon = screen.getByTestId('dt_token_delete_icon');
        await userEvent.click(delete_icon);
    ...
    it('should should trigger deleteToken when clicked on Yes, delete', async () => {
        ...
        await userEvent.click(delete_token_button);
