...
describe('ApiTokenClipboard', () => {
    ...
    it('should display "Copy this token" message when mouse enters', async () => {
        render(<ApiTokenClipboard {...mock_props} />);
        const copy_icon = screen.getByTestId('dt_copy_token_icon');
        await userEvent.hover(copy_icon);
        expect(screen.getByText('Copy this token')).toBeInTheDocument();
    });

    it('should remove "Copy this token" message when mouse leaves', async () => {
        render(<ApiTokenClipboard {...mock_props} />);
        const copy_icon = screen.getByTestId('dt_copy_token_icon');
        await userEvent.hover(copy_icon);
        expect(screen.getByText('Copy this token')).toBeInTheDocument();
        await userEvent.unhover(copy_icon);
        expect(screen.queryByText('Copy this token')).not.toBeInTheDocument();
    });

    it('should display Popup Modal when user clicks on copy_icon', async () => {
        render(<ApiTokenClipboard {...mock_props} />);
        const copy_icon = screen.getByTestId('dt_copy_token_icon');
        await userEvent.click(copy_icon);
        expect(
            screen.getByText(
    ...
    it('should not display Popup Modal when user clicks on copy_icon with no Admin scope', async () => {
