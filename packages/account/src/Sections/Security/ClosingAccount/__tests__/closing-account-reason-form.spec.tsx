...
describe('<ClosingAccountReasonForm />', () => {
    ...
    it('should call the onBackClick function when cancel button is clicked', async () => {
        render(<ClosingAccountReasonForm {...mock_props} />);
    ...
    it('Should be disabled when no reason has been selected', async () => {
        ...
        await userEvent.click(screen.getByRole('checkbox', { name: other_financial_priorities_text }));
        await userEvent.click(screen.getByRole('checkbox', { name: other_financial_priorities_text }));
    ...
    it('should reduce remaining chars from maximum characters count when input is typed', async () => {
        ...
        await userEvent.click(screen.getByLabelText(/i want to stop myself from trading/i));

        await waitFor(() => {
            expect(screen.getByText(/remaining characters: 110/i)).toBeInTheDocument();
        });

        await userEvent.type(screen.getByPlaceholderText(/what could we do to improve/i), 'test suggestion');
        await waitFor(() => {
    ...
    it('should show validation error on invalid characters in input box', async () => {
        ...
        await userEvent.click(screen.getByRole('checkbox', { name: other_financial_priorities_text }));
        await userEvent.type(screen.getByPlaceholderText(/what could we do to improve/i), 'test_suggestion');
