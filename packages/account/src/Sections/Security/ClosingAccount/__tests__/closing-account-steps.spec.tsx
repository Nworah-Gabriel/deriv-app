...
describe('<ClosingAccountSteps />', () => {
    ...
    it('should call redirectToReasons when close_account_button is clicked ', async () => {
        renderComponent();
        const close_account_button = screen.getByRole('button', { name: /close my account/i });
        expect(close_account_button).toBeInTheDocument();
        await userEvent.click(close_account_button);
        expect(mock_props.redirectToReasons).toHaveBeenCalled();
    });

    it('should navigate to root page on clicking the cancel button', async () => {
        renderComponent();
    ...
    it('should render proper button if is_from_derivgo is true', async () => {
