...
describe('DidntGetTheCodeModal', () => {
    ...
    it('should render setShouldShowDidintGetTheCodeModal when Resend code is clicked', async () => {
        renderComponent(VERIFICATION_SERVICES.SMS);
        const resend_code_button = screen.getByRole('button', { name: resend_code_text });
        await userEvent.click(resend_code_button);
        expect(mockSetShouldShowDidntGetTheCodeModal).toHaveBeenCalledTimes(1);
    });

    it('should render mockRequestOnSMS and setOtpVerification with phone_verification_type: sms when Resend code is clicked, phone_verification_type is sms', async () => {
        renderComponent(VERIFICATION_SERVICES.SMS);
        const resend_code_button = screen.getByRole('button', { name: resend_code_text });
        await userEvent.click(resend_code_button);
    ...
    it('should render mockRequestOnWhatsapp and setOtpVerification with phone_verification_type: whatsapp when Resend code is clicked, phone_verification_type is whatsapp', async () => {
        renderComponent(VERIFICATION_SERVICES.WHATSAPP);
        const resend_code_button = screen.getByRole('button', { name: resend_code_text });
        await userEvent.click(resend_code_button);
    ...
    it('should render mockRequestOnSMS and setOtpVerification with phone_verification_type: sms when Send code via SMS is clicked, phone_verification_type is whatsapp', async () => {
        renderComponent(VERIFICATION_SERVICES.WHATSAPP);
    ...
    it('should render mockRequestOnWhatsApp and setOtpVerification with phone_verification_type: whatsapp when Send code via WhatsApp is clicked, phone_verification_type is whatsapp', async () => {
        renderComponent(VERIFICATION_SERVICES.SMS);
