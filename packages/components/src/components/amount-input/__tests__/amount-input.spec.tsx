...
describe('<AmountInput/>', () => {
    ...
    it('should not change the value on non-numeric and non-"." inputs', async () => {
        render(<AmountInput currency='USD' />);
        const input = screen.getByTestId('dt_amount-input');
        await userEvent.type(input, 'abcdef!@#$%^&*()_+-={}[];\'"|\\/,.<>');
        expect(input).toHaveDisplayValue('0.00');
    });

    it('should change the value like an ATM, i.e. from right to left, when entering digits', async () => {
        render(<AmountInput currency='USD' />);
        const input = screen.getByTestId('dt_amount-input');
    ...
    it('should add commas for big values', async () => {
    ...
    it('should not remove "0.00" when backspacing', async () => {
        render(<AmountInput currency='USD' />);
        const input = screen.getByTestId('dt_amount-input');
    ...
    it('should not accept more than {maxDigits} digits', async () => {
        render(<AmountInput currency='USD' max_digits={9} />);
        const input = screen.getByTestId('dt_amount-input');
        await userEvent.type(input, '1234567890987654321');
        expect(input).toHaveDisplayValue('1,234,567.89');
    });

    it('should work correctly with explicitly set {decimal_points}', async () => {
    ...
    it('should allow pasting numbers and then interpret those correctly', () => {
        ...
        values.forEach(async pair => {
            await userEvent.clear(input);
            await userEvent.click(input);
            await userEvent.paste(input, pair[0]);
