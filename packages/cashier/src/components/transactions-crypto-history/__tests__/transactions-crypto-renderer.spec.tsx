...
describe('<TransactionsCryptoRenderer />', () => {
    ...
    it('displays popover when hovering on tooltip for third-party transactions (CoinsPaid) in Desktop mode', async () => {
        const tooltip_props = {
            row: {
                address_hash: 'tb1ql7w62elx9ucw4pj5lgw4l028hmuw80sndtntxt',
        ...
        await userEvent.hover(transactions_crypto_history_table_tooltip);
        expect(screen.getByText('The details of this transaction is available on CoinsPaid.')).toBeInTheDocument();
        await userEvent.unhover(transactions_crypto_history_table_tooltip);
    });
