...
describe('CashierOnboardingOnrampCard', () => {
    describe('user clicks on cashier onramp onboarding card', () => {
        ...
        it('calls `Choose account` modal and navigates to `/cashier/on-ramp` if the user has onramp supported accounts', async () => {
            const wrapper = ({ children }: { children: JSX.Element }) => (
                <CashierProviders store={mock}>{children}</CashierProviders>
            );
            render(<CashierOnboardingOnrampCard />, { wrapper });
        ...
        it('calls `Create account` modal if the user does not have onramp supported accounts', async () => {
