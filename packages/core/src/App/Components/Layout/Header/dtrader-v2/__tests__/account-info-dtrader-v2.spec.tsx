...
describe('AccountInfoDTraderV2', () => {
    ...
    it('should not call toggleDialog if user clicks on account switcher and is_disabled === true', async () => {
        render(<AccountInfoDTraderV2 {...mock_props} is_disabled />);

        expect(mock_props.toggleDialog).not.toBeCalled();
        await userEvent.click(screen.getByText(account_switcher_title));
        expect(mock_props.toggleDialog).not.toBeCalled();
    });

    it('should call toggleDialog if user clicks on account switcher and is_disabled === false', async () => {
        render(<AccountInfoDTraderV2 {...mock_props} />);
    ...
    it('should call toggleDialog with "false" if user clicks outside the modal when it was open', async () => {
        render(<AccountInfoDTraderV2 {...mock_props} />);
