...
describe('AccountListDTraderV2', () => {
    ...
    it('should call onClickResetVirtualBalance is user clicks on "Reset balance" btn', async () => {
        render(<AccountListDTraderV2 {...mock_props} />);

        expect(mock_props.onClickResetVirtualBalance).not.toBeCalled();
        await userEvent.click(screen.getByText(reset_balance_btn));
    ...
    it('should not call redirectAccount if is_disabled === true', async () => {
        render(<AccountListDTraderV2 {...mock_props} is_disabled />);

        expect(mock_props.redirectAccount).not.toBeCalled();
        await userEvent.click(screen.getByText(account_title));
        expect(mock_props.redirectAccount).not.toBeCalled();
    });

    it('should call redirectAccount if is_disabled === false', async () => {
        render(<AccountListDTraderV2 {...mock_props} />);
