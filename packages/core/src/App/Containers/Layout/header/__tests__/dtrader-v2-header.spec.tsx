...
describe('DTraderV2Header', () => {
    ...
    it('should call selectRegion with "EU" if user clicks on Banner and is_eu === true', async () => {
        default_mocked_store.client.has_any_real_account = false;
        default_mocked_store.client.is_landing_company_loaded = true;
        render(mockDTraderV2Header());

        await userEvent.click(screen.getByText(real_signup_banner));

        expect(default_mocked_store.traders_hub.selectRegion).toBeCalledWith('EU');
    });

    it('should call selectRegion with "Non-EU" if user clicks on Banner and is_eu === false', async () => {
        default_mocked_store.client.has_any_real_account = false;
        default_mocked_store.client.is_landing_company_loaded = true;
    ...
    it('should call setShouldShowCooldownModal if user clicks on Banner and real_account_creation_unlock_date is truthy', async () => {
        default_mocked_store.client.has_any_real_account = false;
        default_mocked_store.client.is_landing_company_loaded = true;
