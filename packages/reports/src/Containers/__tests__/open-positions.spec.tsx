...
describe('OpenPositions', () => {
    ...
    it('should render filter dropdown without Accumulators among options when is_eu is true in client-store', async () => {
        ...
        await userEvent.click(screen.getByTestId(filter_dropdown));
    ...
    it('should set Accumulators filter when it is selected from the dropdown on mobile', async () => {
        (useDevice as jest.Mock).mockImplementation(() => ({ isDesktop: false }));
        render(mockedOpenPositions());

        expect(screen.getByRole('combobox')).toHaveValue(options.toLowerCase());
        await userEvent.selectOptions(screen.getByRole('combobox'), accumulators.toLowerCase());
    ...
    it('should set 1% Growth rate filter when it is selected from the dropdown for Accumulators on desktop', async () => {
        ...
        expect(screen.getAllByTestId(filter_dropdown)[1]).toHaveTextContent(all_growth_rates);
        await userEvent.click(screen.getByText(all_growth_rates));
    ...
    it('should set 5% Growth rate filter when it is selected from the dropdown for Accumulators on mobile', async () => {
        ...
        await userEvent.selectOptions(screen.getAllByRole('combobox')[1], five_percent);
